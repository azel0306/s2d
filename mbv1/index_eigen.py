...
parser = argparse.ArgumentParser(description='Eigenvalue computation for model pruning')
parser.add_argument('--custom_model', action='store_true', default=False, help='Use custom model for eigenvalue computation')
...
def eigen(num, layer_num, dataset):
    ...
    if args.custom_model:
        pickle.dump(split_index, open('eigen/custom_{}_min.pkl'.format(num), 'wb'))
        pickle.dump(vector, open('eigen/custom_{}_minv.pkl'.format(num), 'wb'))
        Delta = np.array(Delta)
        pickle.dump(Delta, open('config/custom_delta_{}_{}.pkl'.format(dataset, str(int(num) + 1)), 'wb'))
    else:
