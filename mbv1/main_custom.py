...
if args.custom_model:
    model = model(dataset=args.dataset, activation='relu')
    model.to(device)
else:
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output = model(data)
        if args.dataset == 'rosenbrock' or args.dataset == 'rastrin':
...
def test():
    ...
    for data, target in test_loader:
        ...
        output = model(data)
        if args.dataset == 'rosenbrock' or args.dataset == 'rastrin':
            test_loss += F.mse_loss(output, target, size_average=False).data # sum up batch loss
        else:
